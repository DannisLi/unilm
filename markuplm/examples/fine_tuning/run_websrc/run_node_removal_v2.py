...
def train(args, train_dataset, model, tokenizer):
    ...
    ng_optimizer = ng.optimizers.registry[args.ng_optimizer](
        parametrization=ng.p.Dict(
            ...
            w2 = ng.p.Array(init=tmp[3].weight.detach().cpu().numpy()),
            b2 = ng.p.Array(init=tmp[3].bias.detach().cpu().numpy()),
            w3 = ng.p.Array(init=tmp[6].weight.detach().cpu().numpy()),
            b3 = ng.p.Array(init=tmp[6].bias.detach().cpu().numpy()),
        ), budget=40000, num_workers=args.ng_num_workers)
    del tmp
    ...
    for epoch in train_iterator:
        ...
        for _, batch in enumerate(train_dataloader):
            ...
            def node_removal_objective(params):
                ...
                state_dict['3.weight'] = torch.from_numpy(params['w2'])
                state_dict['3.bias'] = torch.from_numpy(params['b2'])
                state_dict['6.weight'] = torch.from_numpy(params['w3'])
                state_dict['6.bias'] = torch.from_numpy(params['b3'])
            ...
            with torch.no_grad():
                for _ in range(args.ng_steps//2):
                    t1 = ng_optimizer.ask()
                    t2 = ng_optimizer.ask()
                    loss_t1 = node_removal_objective(t1.value)
                    loss_t2 = node_removal_objective(t2.value)
                    ng_optimizer.tell(t1, loss_t1)
                    ng_optimizer.tell(t2, loss_t2)
            ...
            state_dict['3.weight'] = torch.from_numpy(node_removal_params['w2'])
            state_dict['3.bias'] = torch.from_numpy(node_removal_params['b2'])
            state_dict['6.weight'] = torch.from_numpy(node_removal_params['w3'])
            state_dict['6.bias'] = torch.from_numpy(node_removal_params['b3'])
            model.module.node_removal_layer.load_state_dict(state_dict)
            
            del state_dict, t1, t2, loss_t1, loss_t2, node_removal_params, inputs_for_ng
            ...
            for i in range(math.ceil(batch[0].size(0) / args.train_batch_size)):
                inputs = {
                    ...
                    'num_nodes': batch[8][i*args.train_batch_size:(i+1)*args.train_batch_size],
                    'query_span': batch[9][i*args.train_batch_size:(i+1)*args.train_batch_size],
                    'constraints': batch[10][i*args.train_batch_size:(i+1)*args.train_batch_size]}

                outputs = model(**inputs)
                # loss = outputs[0]  # model outputs are always tuple in transformers (see doc)
                loss, QA_loss, R_len, R_tree = outputs
                step += 1
                ...
                if step % 20 == 0:
                    print(
                        ...
                        "QA_loss:", round(QA_loss.item(), 5), 
                        "R_len:", round(R_len.item(), 2), 
                        "R_tree:", round(R_tree.item(), 3), 
...
def load_and_cache_examples(args, tokenizer, max_depth=50, evaluate=False, output_examples=False):
    ...
    all_num_nodes = []
    all_constraints = []    # node score constraints: [max_num_nodes]
    # all_intersect_with_answer = []
    for f in features:
        node_spans = f.node_spans
        num_nodes = len(node_spans)
        ...
        cnt = 0
        ##########################
        # 如果num_nodes <= max_num_nodes, 将全部nodes放入train/test data，然后padding
        # 如果num_nodes > max_num_nodes, 将不放回抽取max_num_nodes个nodes放入train/test data
        if num_nodes <= args.max_num_nodes:
            _node_spans.extend(node_spans)
            _node_ids = list(range(len(node_spans)))
            # _intersect_with_answer.extend(intersect_with_answer)
            cnt = num_nodes
            while cnt < args.max_num_nodes:
                _node_spans.append(node_spans[-1])
                _node_ids.append(_node_ids[-1])
                # _intersect_with_answer.append(-100)
                cnt += 1
        else:
            _node_ids = np.random.choice(num_nodes, size=args.max_num_nodes, replace=False).tolist()
            _node_spans.extend([node_spans[i] for i in _node_ids])
            # _intersect_with_answer.extend([intersect_with_answer[i] for i in chosen])
        _num_nodes = min(num_nodes, args.max_num_nodes)
        # node removal layer的节点重要性分数的树型结构约束
        _constraints = []
        for _id in _node_ids:
            try:
                _constraints.append(_node_ids.index(f.father_ids[_id]))
            except ValueError:
                _constraints.append(-1)
        # 添加到全体
        all_node_spans.append(_node_spans)
        all_num_nodes.append(_num_nodes)
        # all_intersect_with_answer.append(_intersect_with_answer)
        all_constraints.append(_constraints)
    ...
    if evaluate:
        ...
        all_constraints = torch.tensor(all_constraints, dtype=torch.long)
        all_start_positions = torch.tensor([f.start_position for f in features], dtype=torch.long)
        all_end_positions = torch.tensor([f.end_position for f in features], dtype=torch.long)
        dataset = StrucDataset(
            all_input_ids, all_input_mask, all_segment_ids,
            ...
            all_query_span, all_constraints)
...
def parse():
    ...
    parser.add_argument(
        '--ng_optimizer', 
        choices=[
            "TwoPointsDE", "RandoSearmch", "TBPSA", "CMA", "NaiveTBPSA", "OnePlusOne", 
            "NoisyOnePlusOne", "NGOpt", "RealSpacePSO", "OptimisticNoisyOnePlusOne", 
            "NelderMead", "BO", "BayesOptimBO"], 
    ...
    parser.add_argument('--ng_num_workers', type=int, default=8, help="")
    # parser.add_argument('--length_regularization', type=float, default=1e-3, help="")
